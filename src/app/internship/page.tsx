  ...
  return (
    <main className="bg-slate-50 text-slate-800 dark:bg-transparent dark:text-slate-200 container px-4">
      <div className="max-w-[1600px] mx-auto">
        {/* --- Header Section (ปรับให้คล้ายกับ Announcement แต่ใช้สีเขียว) --- */}
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6 border-b border-slate-200 pb-6 dark:border-slate-800">
          <div className="space-y-2 border-l-4 border-emerald-500 pl-4">
            <div className="flex items-center gap-2 text-emerald-600 font-bold uppercase tracking-widest text-xs dark:text-emerald-400">
            ...
            <p className="text-slate-500 text-sm md:text-base max-w-lg dark:text-slate-400 font-medium">
              ข้อมูลความเคลื่อนไหว การนิเทศ และกิจกรรมของนักศึกษาในสถานประกอบการ
            </p>
          </div>

          {/* ปุ่ม Link ไปยังหมวดหมู่ทั้งหมด (ถ้ามี) */}
          <Link
            href="/news?category=Internship"
            className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-full font-bold text-sm shadow-md shadow-emerald-100 hover:bg-emerald-700 transition-all hover:shadow-lg active:scale-95 group dark:shadow-none dark:bg-emerald-500"
          >
            <svg
              className="w-4 h-4"
              fill="none"
        ...
        {internshipData.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 rounded-3xl border-2 border-dashed border-slate-200 text-slate-400 dark:border-slate-800 dark:text-slate-600">
            <p className="text-lg font-semibold">ไม่พบข้อมูลการฝึกประสบการณ์</p>
          </div>
        )}
      </div>
    </main>
