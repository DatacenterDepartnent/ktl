...
export default function WorkReportsManagementPage() {
  ...
  const handleEditInit = (report: any) => {
    ...
    setEditActivities(report.activities?.map((a: any) => ({ ...a, id: a.id || crypto.randomUUID() })) || []);
  ...
  const handleDelete = async (id: string) => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`/api/work-report?id=${id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) {
  ...
  const addActivity = () => {
    setEditActivities([{ id: crypto.randomUUID(), taskName: "", detail: "", status: 'Completed' }, ...editActivities]);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-neutral-950 py-8 px-4 font-sans selection:bg-indigo-500/30">
      <div className="max-w-7xl mx-auto space-y-8">
        
        {/* Header Section */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-neutral-900 p-8 rounded-[3rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-neutral-800">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-800 dark:text-neutral-100 uppercase tracking-tight">Work Reports Management</h1>
              <p className="text-sm text-slate-500 dark:text-neutral-400 font-bold mt-1.5 uppercase tracking-widest opacity-70">Super Admin Only • Edit & Delete Controls</p>
            </div>
          </div>
          
          <button onClick={fetchReports} className="flex items-center gap-2 px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl shadow-md text-sm font-black hover:scale-105 transition-all active:scale-95">
             <Clock size={18} className={loading ? "animate-spin" : ""} /> Refresh Data
          </button>
        </div>

        {/* Filter Section */}
        <div className="bg-white dark:bg-neutral-900 p-8 rounded-[3rem] shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-neutral-800 grid grid-cols-1 md:grid-cols-4 gap-8 items-end">
          <div className="md:col-span-2">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-2">Search Employee / Dept</label>
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-2">Start Date</label>
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-2">End Date</label>
            <div className="relative">
              <Calendar className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" size={18} />
              <input
                type="date"
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-[3rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-neutral-800 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-neutral-800/50">
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 dark:border-neutral-800">Employee</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 dark:border-neutral-800">Date</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 dark:border-neutral-800">Summary</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 dark:border-neutral-800">Tasks</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 dark:border-neutral-800 text-right">Actions</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={5} className="py-20 text-center">
                        ...
                        <p className="text-slate-400 font-black uppercase tracking-widest text-xs">กำลังโหลดข้อมูลจัดการรายงาน...</p>
                      </td>
                    </tr>
                      ...
                      >
                        <td className="px-8 py-6 border-b border-slate-50 dark:border-neutral-800">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-slate-800 dark:text-neutral-200">{report.user.name}</p>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mt-1">{report.user.department}</p>
                            </div>
                          </div>
                        </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex flex-col h-[90vh]">
                {/* Modal Header */}
                <div className="p-8 border-b border-slate-100 dark:border-neutral-800 flex items-center justify-between bg-slate-50/50 dark:bg-neutral-800/50">
                  ...
                  <button onClick={() => setIsEditing(false)} className="p-3 hover:bg-white dark:hover:bg-neutral-800 rounded-2xl text-slate-400 hover:text-rose-500 transition-all">
                    <X size={24} />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="md:col-span-2 space-y-2">
                               <input 
                                 ...
                                 value={act.taskName}
                                 onChange={(e) => updateActivityField(act.id, 'taskName', e.target.value)}
                               ...
                               <textarea 
                                 ...
                                 value={act.detail}
                                 onChange={(e) => updateActivityField(act.id, 'detail', e.target.value)}
                               ...
                               {(['Completed', 'In Progress', 'Pending'] as const).map(s => (
                                 <button 
                                   ...
                                   className={`py-2 rounded-xl text-[10px] font-black uppercase transition-all border ${
                                     act.status === s 
                                      ? s === 'Completed' ? 'bg-emerald-500 border-emerald-500 text-white' : s === 'In Progress' ? 'bg-blue-500 border-blue-500 text-white' : 'bg-amber-500 border-amber-500 text-white'
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Overall Summary</label>
                        <textarea 
                           value={editSummary}
                     ...
                     <div className="space-y-6">
                        <div className="space-y-3">
                           <label className="text-[10px] font-black text-rose-400 uppercase tracking-widest pl-2">Problems Found</label>
                           <textarea 
                              value={editProblems}
                        ...
                        <div className="space-y-3">
                           <label className="text-[10px] font-black text-emerald-400 uppercase tracking-widest pl-2">Next Day Plan</label>
                           <textarea 
                ...
                {/* Modal Footer */}
                <div className="p-8 border-t border-slate-100 dark:border-neutral-800 flex justify-end gap-4 bg-slate-50/50 dark:bg-neutral-800/50">
                  <button 
                    onClick={() => setIsEditing(false)}
