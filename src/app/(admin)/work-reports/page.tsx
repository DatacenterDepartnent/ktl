...
export default function AdminWorkReportsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-neutral-950 py-8 px-4 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-neutral-900 p-6 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-neutral-800">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-800 dark:text-neutral-100 uppercase tracking-tight">ระบบรายงานการปฏิบัติงาน</h1>
              <p className="text-sm text-slate-500 font-medium mt-1">Review and monitor daily individual work reports</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 bg-slate-50 dark:bg-neutral-800 p-1.5 rounded-2xl border border-slate-100 dark:border-neutral-700">
             <button onClick={fetchReports} className="px-4 py-2 bg-white dark:bg-neutral-900 text-slate-800 dark:text-white rounded-xl shadow-sm text-xs font-bold hover:bg-slate-50 transition-all active:scale-95">
                Refresh Data
             </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-neutral-800 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          <div className="md:col-span-2">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1">Search Employee / Dept</label>
          ...
          <AnimatePresence mode="popLayout">
            {loading ? (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">กำลังโหลดข้อมูลรายงาน...</p>
              </div>
              ...
              <div className="col-span-full py-20 text-center bg-white dark:bg-neutral-900 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-neutral-800">
                ...
                <p className="text-slate-400 font-bold">ไม่พบข้อมูลรายงานการปฏิบัติงานในช่วงเวลาที่เลือก</p>
              </div>
            ) : (
              filteredReports.map((report) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h3 className="font-black text-slate-800 dark:text-neutral-100 text-lg leading-tight">{report.user.name}</h3>
                      <p className="text-xs font-bold text-slate-400 dark:text-neutral-500 uppercase tracking-widest leading-none mt-1">{report.user.department}</p>
                    </div>
                  </div>
              ...
              <div className="p-8 space-y-8 overflow-y-auto max-h-[90vh]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2 space-y-6">
                    <section>
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 border-b border-slate-100 pb-2">Activities Detail</h4>
                      <div className="space-y-4">
                        {selectedReport.activities?.map((act: any, idx: number) => (
                          <div key={idx} className="flex gap-4">
                            <div className="flex-none pt-1">
                              {act.status === 'Completed' ? <CheckCircle2 size={18} className="text-emerald-500" /> : <Clock size={18} className="text-blue-500" />}
                            </div>
                            <div>
                              <p className="font-bold text-slate-800 dark:text-neutral-200 text-sm">{act.taskName}</p>
                              {act.detail && <p className="text-xs text-slate-500 dark:text-neutral-400 mt-1 leading-relaxed">{act.detail}</p>}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="space-y-6">
                    <section className="p-5 bg-slate-50 dark:bg-neutral-800/50 rounded-3xl border border-slate-100 dark:border-neutral-800">
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Overall Summary</h4>
                      <p className="text-xs text-slate-700 dark:text-neutral-300 leading-relaxed italic">"{selectedReport.summary}"</p>
                    </section>
                    ...
                    {selectedReport.problems && (
                      <section className="p-5 bg-rose-50 dark:bg-rose-900/10 rounded-3xl border border-rose-100 dark:border-rose-900/20">
                        <h4 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.2em] mb-3">Problems Found</h4>
                        <p className="text-xs text-rose-700 dark:text-rose-300 leading-relaxed">{selectedReport.problems}</p>
                      </section>
                    )}
                    ...
                    {selectedReport.plansNextDay && (
                      <section className="p-5 bg-emerald-50 dark:bg-emerald-900/10 rounded-3xl border border-emerald-100 dark:border-emerald-900/20">
                        <h4 className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] mb-3">Next Day Plan</h4>
                        <p className="text-xs text-emerald-700 dark:text-emerald-300 leading-relaxed">{selectedReport.plansNextDay}</p>
                      </section>
                    )}
                  </div>
                </div>
