...
import { ArrowLeft, Calendar, FileText, Upload, Send, HelpCircle, Loader2 } from "lucide-react";
...
export default function LeaveRequestPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-4 selection:bg-blue-500/30 font-sans">
      {/* Background Decor */}
      ...
      {success ? (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-4xl shadow-2xl overflow-hidden relative z-10">
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
            <div className="space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest pl-1">ประเภทการลา (Leave Type)</label>
                <select
              ...
              >
                {loading ? (
                  <>
                    ...
                    <span className="relative z-10">กำลังส่งคำขอ...</span>
                  </>
                ) : (
                  <>
                    <span className="relative z-10">ประทับตรายืนยันสถานะลางาน</span>
