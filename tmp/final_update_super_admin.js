...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('<optgroup label="๔. ฝ่ายวิชาการ (ปวช.)">')) {
        startIndex = i;
    }
    // We want to find the NEXT </optgroup> that closes this section
    if (startIndex !== -1 && lines[i].includes('</optgroup>') && i > startIndex) {
        endIndex = i;
        break;
    }
}
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    lines.splice(startIndex, (endIndex - startIndex + 1), ...newOptions);
    fs.writeFileSync(path, lines.join('\n'), 'utf8');
    console.log("Updated super-admin successfully at indices " + startIndex + " to " + endIndex);
} else {
