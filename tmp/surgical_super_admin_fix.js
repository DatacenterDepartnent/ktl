...
// Find the start and end of the department select based on unique nearby text
const startMarker = '<td className="p-4 text-center">\r\n                        <select\r\n                          value={user.department || "ไม่มีสังกัด"}';
const endMarker = '</select>';

// Since line endings might vary, I'll use a more flexible regex
const selectRegex = /<select\s+value=\{user\.department\s+\|\|\s+"ไม่มีสังกัด"\}[\s\S]*?<\/select>/;
...
if (content.match(selectRegex)) {
    const updatedContent = content.replace(selectRegex, newSelect);
    fs.writeFileSync(path, updatedContent, 'utf8');
    console.log("Successfully replaced the department select.");
} else {
    console.error("Could not find the select element with the expected value pattern.");
}
